...
try{
	...
	# copy extra files to $packOutput before calling publish
	$exfilesdir = 'C:\Data\personal\mycode\orlandocc-2015\PublishSamples\ExtraFiles'
    'copying extra files from [{0}] to [{1}]' -f $exfilesdir,$packOutput | Write-Output
    Get-ChildItem -Path $exfilesdir | Copy-Item -Destination "$packOutput\wwwroot\images"
